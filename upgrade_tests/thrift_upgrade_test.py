...
def _validate_sparse_cql(cursor, cf='sparse_super_1', column1='column1', col1='col1', col2='col2', key='key', is40=False):
...
def _validate_dense_cql(cursor, cf='dense_super_1', key='key', column1='column1', column2='column2', value='value', is40=False):
    cursor.execute('use ks')
    ...
    if is40:
        ...
        for dict in expected:
            del dict[column2]
    original_iterator = cursor.execute("SELECT * FROM {}".format(cf))
    original_result = list(original_iterator)
    result = unpack(original_result)
    assert_lists_of_dicts_equal(result, expected)
...
class TestUpgradeSuperColumnsThrough(Tester):
    ...
    def test_dense_supercolumn_3_0_created(self):
        ...
        if node.get_cassandra_version() < '4':
           ...
           node.set_configuration_options(values={'start_rpc': None}, delete_empty=True)
        node.start()
        ...
        if node.get_cassandra_version() < '4':
            client = get_thrift_client(host, port)
            _validate_dense_thrift(client, cf='dense_super_1')
        _validate_dense_cql(cursor, cf='dense_super_1', is40=node.get_cassandra_version() >= '4')
    ...
    def test_sparse_supercolumn(self):
        ...
        self.set_node_to_current_version(node)
        is40 = node.get_cassandra_version() >= '4'
        #4.0 doesn't support compact storage
        if is40:
        ...
        cursor = self.patient_cql_connection(node, row_factory=dict_factory)
        _validate_sparse_cql(cursor, cf='sparse_super_2', is40=is40)
...
class TestThrift(UpgradeTester):
    ...
    def test_dense_supercolumn(self):
        ...
        is40 = version_string == 'trunk' or version_string >= '4.0'
        #4.0 doesn't support compact storage
        if is40:
            cursor.execute("ALTER TABLE ks.dense_super_1 DROP COMPACT STORAGE;")
        ...
        for is_upgraded, cursor in self.do_upgrade(cursor, row_factory=dict_factory, use_thrift=True):
            ...
            if not is40:
                client = get_thrift_client(host, port)
                _validate_dense_thrift(client)
            _validate_dense_cql(cursor, is40=is40)
    ...
    def test_dense_supercolumn_with_renames(self):
        ...
        for is_upgraded, cursor in self.do_upgrade(cursor, row_factory=dict_factory, use_thrift=True):
            ...
            if not is40:
                client = get_thrift_client(host, port)
                _validate_dense_thrift(client, cf='dense_super_2')
            _validate_dense_cql(cursor, cf='dense_super_2', key='renamed_key', column1='renamed_column1', column2='renamed_column2', value='renamed_value', is40=is40)
    ...
    def test_sparse_supercolumn_with_renames(self):
        ...
        for is_upgraded, cursor in self.do_upgrade(cursor, row_factory=dict_factory, use_thrift=True):
            ...
            if not is40:
                client = get_thrift_client(host, port)
                _validate_sparse_thrift(client)
            _validate_sparse_cql(cursor, column1='renamed_column1', key='renamed_key', is40=is40)
